...
def multiclass_classification_objective(valuation_index):
    if valuation_index in ['f1_macro', 'f1_micro', 'f1_weighted', 'accuracy', 'precision_macro', 'recall_macro', 'roc_auc']:
        return 'maximize'
    elif valuation_index == 'logloss':
        return 'minimize'
    else:
        raise ValueError(f'valuation_index: {valuation_index} is not directly supported for multi-class in this snippet')
